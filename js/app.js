...
async function openIssueDetails(id) {
    const issue = await API.fetchById(id);
    if (!issue) return;
    document.getElementById('modalTitle').innerText = issue.title;
    document.getElementById('modalDesc').innerText = issue.description;
    document.getElementById('modalAssignee').innerText = issue.author || "Fahim Ahmed";
    document.getElementById('modalMeta').innerText = `• Opened by ${issue.author} • March 9, 2026`;
    const statusB = document.getElementById('modalStatus');
    const isModalOpen = (issue.status || "").toLowerCase() === 'open';
    statusB.innerText = isModalOpen ? 'Opened' : 'Closed';
    statusB.className = `rounded-full px-4 py-0.5 text-white text-[10px] font-bold uppercase mb-4 inline-block ${isModalOpen ? 'bg-[#00BA88]' : 'bg-[#9333EA]'}`;
